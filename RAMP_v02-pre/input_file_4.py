...
HI_indoor_bulb = HI.Appliance(HI,n=8,P=7,w=2,t=180,r_t=0.3,c=15)
...
HI_TV = HI.Appliance(HI,2,80,1,180,0.3,5)
HI_TV.windows([1020,1320],r_w=0.3)
...
HMI_TV = HMI.Appliance(HMI,1,50,1,180,0.3,5)
HMI_TV.windows([1020,1440],r_w=0.3)
...
Ho_indoor_bulb.windows([1080,1320],[330,390],0.1)
...
Ho_Freezer = Hospital.Appliance(Hospital,1,140,1,1440,0,30,'yes',3)
...
Ho_washing_mac.windows([480,720],[870,1440],0)

#Tailor
Ta_indoor_bulb = Tailor.Appliance(Tailor,6,10,2,240,0.3,60,wd_we_type = 0)
Ta_indoor_bulb.windows([600,720],[900,1080],0.1)

Ta_sewing_mac = Tailor.Appliance(Tailor,1,100,2,240,0.5,30,wd_we_type = 0)
Ta_sewing_mac.windows([600,720],[900,1080],0)

#Carpenter
Ca_jigsaw = Carpenter.Appliance(Carpenter,1,2000,2,120,0.3,5,wd_we_type = 0,occasional_use = 0.33) #??
Ca_jigsaw.windows([600,720],[900,1080],0)

Ca_driller = Carpenter.Appliance(Carpenter,1,1500,2,60,0.5,1,wd_we_type = 0,occasional_use = 0.33)
Ca_driller.windows([600,720],[900,1080],0)

Ca_planer = Carpenter.Appliance(Carpenter,1,600,2,60,0.5,5,wd_we_type = 0,occasional_use = 0.33)
Ca_planer.windows([600,720],[900,1080],0)

Ca_indoor_bulb = Carpenter.Appliance(Carpenter,6,10,2,240,0.3,60,wd_we_type = 0)
Ca_indoor_bulb.windows([600,720],[900,1080],0.1)

#Garage
Ga_indoor_bulb = Garage.Appliance(Garage,10,10,2,240,0.3,60,wd_we_type = 0)
...
Ga_welding_mac.windows([600,720],[900,1080],0)

#Bar
Ba_indoor_bulb = Bar.Appliance(Bar,6,10,2,240,0.3,60)
Ba_indoor_bulb.windows([960,1260],r_w=0.1)

Ba_fridge = Bar.Appliance(Bar,1,140,1,flat='yes')
Ba_fridge.windows([0,1440],r_w=0)

Ba_hifi = Bar.Appliance(Bar,1,60,2,180,0.3,30)
Ba_hifi.windows([600,720],[900,1260],0.1)
...
#School
S_indoor_bulb = School.Appliance(School,30,10,1,240,0.3,60,wd_we_type = 0)
S_indoor_bulb.windows([960,1200],r_w=0.2)

S_outdoor_bulb = School.Appliance(School,5,1,10,2,flat='yes')
S_outdoor_bulb.windows([1080,1440],r_w=0.1)
