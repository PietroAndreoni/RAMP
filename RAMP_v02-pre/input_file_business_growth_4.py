...
HI = User("high income",230,3)
User_list.append(HI)

HMI = User("higher middle income",520,3)
User_list.append(HMI)

LI = User("low income",160,3)
User_list.append(LI)

Hospital = User("hospital",2)
User_list.append(Hospital)

School = User("school",3)
...
#New businesses
Laundry = User("laundry",1)
User_list.append(Laundry)

Internet_point = User("internet point",1)
User_list.append(Internet_point)

Grocery_store = User("grociery store",2)
User_list.append(Grocery_store)

Haircutter = User("haircutter",2)
User_list.append(Haircutter)

Generic_business = User("generic business",4)
User_list.append(Generic_business)

#create new appliances

#Public lighting
Pub_lights = Public_lighting.Appliance(Public_lighting,n=130,P=50,w=1,fixed='yes',flat='yes') ###????
...
HI_TV2.windows([720,1080],r_w=0.1)
...
HI_Blender = HI.Appliance(HI,1,50,2,20,0.3,1,occasional_use = 0.5) #ch from 5 to 3
...
HMI_TV2.windows([720,1080],r_w=0.1)
...
LI_indoor_bulb2 = LI.Appliance(LI,4,7,2,20,0.2,5)
...
#Hospital
Ho_indoor_bulb = Hospital.Appliance(Hospital,40,10,2,180,0.3,15)
Ho_indoor_bulb.windows([480,780],[870,1440],0.1)

Ho_outdoor_bulb = Hospital.Appliance(Hospital,2,10,2,fixed='yes',flat='yes')
Ho_outdoor_bulb.windows([1080,1440],[0,390],0.1)

Ho_water_heater = Hospital.Appliance(Hospital,1,800,2,60,0.5,20)
Ho_water_heater.windows([480,780],[870,1440],0)

Ho_Sterilizer = Hospital.Appliance(Hospital,n=1,P=1500,w=2,t=60,occasional_use=0.5)
Ho_Sterilizer.windows([480,780],[870,1440],0)
...
Ta_indoor_bulb.windows([600,780],[900,1080],0.1)

Ta_sewing_mac = Tailor.Appliance(Tailor,1,100,2,240,0.5,30,wd_we_type = 0)
Ta_sewing_mac.windows([600,780],[900,1080],0)

#Carpenter
Ca_jigsaw = Carpenter.Appliance(Carpenter,1,2000,2,120,0.5,5,wd_we_type = 0,occasional_use = 0.5) #??
Ca_jigsaw.windows([600,780],[900,1080],0.1)

Ca_driller = Carpenter.Appliance(Carpenter,1,1500,2,90,0.5,1,wd_we_type = 0,occasional_use = 0.5)
Ca_driller.windows([600,780],[900,1080],0.1)

Ca_planer = Carpenter.Appliance(Carpenter,1,600,2,60,0.5,5,wd_we_type = 0,occasional_use = 0.5)
Ca_planer.windows([600,780],[900,1080],0.1)

Ca_indoor_bulb = Carpenter.Appliance(Carpenter,6,10,2,300,0.3,60,wd_we_type = 0)
Ca_indoor_bulb.windows([600,780],[900,1080],0.1)

#Garage
Ga_indoor_bulb = Garage.Appliance(Garage,10,10,2,300,0.3,60,wd_we_type = 0)
Ga_indoor_bulb.windows([600,780],[900,1080],0.1)

Ga_welding_mac = Garage.Appliance(Garage,1,1500,2,120,0.5,5,wd_we_type = 0,occasional_use = 0.5)
Ga_welding_mac.windows([600,780],[900,1080],0.1)

#Bar
Ba_indoor_bulb = Bar.Appliance(Bar,6,10,2,300,0.3,60)
Ba_indoor_bulb.windows([960,1440],r_w=0.1)

Ba_fridge = Bar.Appliance(Bar,1,140,1,1440,0,30,'yes',3)
HI_Freezer.windows([0,1440],[0,0])
HI_Freezer.specific_cycle_1(200,20,5,10)
HI_Freezer.specific_cycle_2(200,15,5,15)
HI_Freezer.specific_cycle_3(200,10,5,20)
...
Ba_hifi = Bar.Appliance(Bar,1,60,1,180,0.1,30)
Ba_hifi.windows([960,1440],r_w=0.1)
...
#School
S_indoor_bulb = School.Appliance(School,30,10,2,300,0.3,60,wd_we_type = 0)
S_indoor_bulb.windows([540,720],[900,1200])

S_outdoor_bulb = School.Appliance(School,5,2,10,2,flat='yes')
